...
class _DashboardState extends State<Dashboard> {
  ...
  String? _selectedProfileName = prefs.getString(
    "cache.app.selectedProfileName",
  );

  bool _highlightSelectProfile = false;
  final bool _useFloatingActionButton = prefs.getBool(
  ...
  Future<void> _loadSettings() async {
    ...
    if (selectedProfileId != null) {
      final selectedProfile = await (db.select(
        db.profile,
      )..where((p) => p.id.equals(selectedProfileId))).getSingleOrNull();
      if (mounted) {
        setState(() {
